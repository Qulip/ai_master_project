...
from conf.settings import get_embeddings, get_llm
import os


def pdf_text_splitter_faiss_indexing(embeddings, query):
    """
    ...
    if os.path.exists("faiss/pdf_faiss_index"):
        vectorstore = load_faiss_vector_store("faiss/pdf_faiss_index", embeddings)
    else:
        docs = load_pdf()
        chunks = character_text_splitter(docs)
        vectorstore = create_faiss_vector_store(chunks, embeddings)
        save_faiss_vector_store(vectorstore, "faiss/pdf_faiss_index")

    search_faiss_vector_store(vectorstore, query)
...
def __main__():
    ...
    pdf_text_splitter_faiss_indexing(embeddings, "2025 고용 전망은 어떠한가?")
